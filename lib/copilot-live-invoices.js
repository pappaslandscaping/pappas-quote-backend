...
const { cleanText } = require('./copilot-invoice-pdf');
const { matchesInvoiceCustomer, normalizedMatchValue } = require('./copilot-customer-match');
...
function isDraftInvoice(invoice) {
  const status = String(invoice?.status || invoice?.raw_status || '').trim().toLowerCase();
...
function extractMoneyNearLabel(text, label) {
  const patterns = [
    new RegExp(`${label}[\\s\\S]{0,120}?([0-9][0-9,]*\\.\\d{2})`, 'i'),
    new RegExp(`([0-9][0-9,]*\\.\\d{2})[\\s\\S]{0,80}?${label}`, 'i'),
  ];

  for (const pattern of patterns) {
    const match = String(text || '').match(pattern);
    if (match) {
      const numeric = parseMoneyValue(match[1]);
...
function parseCopilotCustomerStandingHtml(html) {
  const $ = cheerio.load(html || '');
  const bodyText = cleanText($('body').text());

  let pastDue = extractMoneyNearLabel(bodyText, 'Past Due');
  let outstanding = extractMoneyNearLabel(bodyText, 'Outstanding');
  let paid = extractMoneyNearLabel(bodyText, 'Paid');
  let credit = extractMoneyNearLabel(bodyText, 'Credit');

  $('div, span, p').each((_, el) => {
    const text = cleanText($(el).text());
    if (!text) return;

    if (pastDue === null && /past due/i.test(text)) {
      const nearby = cleanText(`${text} ${$(el).parent().text()} ${$(el).next().text()}`);
      pastDue = extractMoneyNearLabel(nearby, 'Past Due');
    }
    if (outstanding === null && /outstanding/i.test(text)) {
      const nearby = cleanText(`${text} ${$(el).parent().text()} ${$(el).next().text()}`);
      outstanding = extractMoneyNearLabel(nearby, 'Outstanding');
    }
    if (paid === null && /^paid$/i.test(text)) {
      const nearby = cleanText(`${text} ${$(el).parent().text()} ${$(el).next().text()}`);
      paid = extractMoneyNearLabel(nearby, 'Paid');
    }
    if (credit === null && /^credit$/i.test(text)) {
      const nearby = cleanText(`${text} ${$(el).parent().text()} ${$(el).next().text()}`);
      credit = extractMoneyNearLabel(nearby, 'Credit');
    }
  });
...
async function fetchCopilotCustomerStanding(settings, customerId) {
  const normalizedCustomerId = String(customerId || '').trim();
  if (!normalizedCustomerId) return null;

  const response = await fetchCopilot(settings, `/customers/details/${normalizedCustomerId}`, {
    accept: 'text/html,application/xhtml+xml',
    xRequestedWith: '',
  });
  const html = await response.text();
  if (isCopilotLoginPage(html, response.url)) {
    throw new Error(`Copilot session expired while loading customer detail ${normalizedCustomerId}`);
  }

  return parseCopilotCustomerStandingHtml(html);
}
...
async function discoverInvoicePdf(settings, invoice) {
  ...
  const htmlSnapshot = {
    ...
    metadata: {
      ...
      customer_name: parsedFromHtml?.customer_name || invoice.customer_name || null,
      invoice_date: parsedFromHtml?.invoice_date || null,
      detail_page_url: pageUrl,
      detail_pdf_url: pdfUrlUsed,
      detail_fetch_errors: errors,
      detail_fetch_mode: 'html-only',
    },
  };

  const htmlInvoice = mergeInvoiceSnapshots(invoice, htmlSnapshot);
  const parsedInvoice = htmlInvoice;

  const customerStanding = await fetchCopilotCustomerStanding(
    settings,
    parsedInvoice?.copilot_customer_id
      || parsedFromHtml?.copilot_customer_id
      || invoice?.copilot_customer_id
      || invoice?.metadata?.copilot_customer_id
  ).catch(() => null);

  if (customerStanding) {
    const standingMeta = {
      customer_past_due_balance: customerStanding.past_due_balance,
      customer_outstanding_balance: customerStanding.outstanding_balance,
      customer_paid_balance: customerStanding.paid_balance,
      customer_credit_available: customerStanding.credit_available,
    };

    if (customerStanding.past_due_balance !== null) {
      standingMeta.past_due_balance = customerStanding.past_due_balance;
    }
    if (customerStanding.outstanding_balance !== null) {
      const accountOutstanding = Number(customerStanding.outstanding_balance);
      const invoiceTotal = Number(parsedInvoice.total || 0);
      standingMeta.prior_balance = accountOutstanding;
      standingMeta.outstanding_balance = accountOutstanding;
      standingMeta.total_due = accountOutstanding + invoiceTotal;
      standingMeta.total_due_on_account = accountOutstanding + invoiceTotal;
    }
...
async function syncCopilotInvoices({
  ...
  const invoicesToSync = listPayload.invoices.filter((invoice) => matchesInvoiceCustomer(invoice, {
    customerName,
    customerEmail,
    copilotCustomerId,
  }));
  ...
  for (const summaryInvoice of invoicesToSync) {
    try {
      if (isDraftInvoice(summaryInvoice)) {
        skipped.push({
          reason: 'draft',
          external_invoice_id: summaryInvoice.external_invoice_id,
          invoice_number: summaryInvoice.invoice_number,
          customer_name: summaryInvoice.customer_name || null,
        });
        continue;
      }
      ...
      if (isDraftInvoice(detailedInvoice)) {
        skipped.push({
          reason: 'draft',
          external_invoice_id: summaryInvoice.external_invoice_id,
          invoice_number: detailedInvoice.invoice_number || summaryInvoice.invoice_number,
          customer_name: detailedInvoice.customer_name || summaryInvoice.customer_name || null,
        });
        continue;
      }
...
function normalizeInvoiceFinancialRow(row) {
  ...
  return {
    ...
    external_metadata: {
      ...metadata,
      outstanding_balance: accountSummary.priorBalance,
      this_invoice: total,
      total_due: accountSummary.totalDueOnAccount,
      total_due_on_account: accountSummary.totalDueOnAccount,
    },
  };
}
...
module.exports = {
  ...
  parseCopilotCustomerStandingHtml,
  refreshCopilotInvoiceSnapshot,
  saveCopilotSettings,
  syncCopilotInvoices,
  _internal: {
    matchesInvoiceCustomer,
