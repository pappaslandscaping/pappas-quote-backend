...
function parseCopilotPaymentDetailHtml(html) {
  const $ = cheerio.load(html || '');
  const text = cleanText($('body').text());
  const methodMatch = text.match(/Payment Method\s+(.+?)(?=\s+(?:Payer|Tip|Details|Payment Notes|$))/i);
  const method = cleanText(methodMatch?.[1]);
  const checkMatch = method.match(/check\s*#\s*([a-z0-9-]+)/i)
    || text.match(/Payment Method\s+Check\s*#\s*([a-z0-9-]+)/i);
  return {
    method: method || null,
