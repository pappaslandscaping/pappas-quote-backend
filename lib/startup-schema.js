...
const { seedDispatchRouteTemplates } = require('./dispatch-route-template-seeds');
...
const PAYMENTS_TABLE = `CREATE TABLE IF NOT EXISTS payments (
  ...
  external_payment_key VARCHAR(120),
...
const COPILOT_TAX_SUMMARY_SNAPSHOTS_TABLE = `CREATE TABLE IF NOT EXISTS copilot_tax_summary_snapshots (
  id SERIAL PRIMARY KEY,
  external_source VARCHAR(40) NOT NULL DEFAULT 'copilotcrm',
  basis VARCHAR(30) NOT NULL DEFAULT 'collected',
  ...
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  UNIQUE (external_source, basis, start_date, end_date)
)`;

const TAX_TRANSFER_INSTRUCTIONS_TABLE = `CREATE TABLE IF NOT EXISTS tax_transfer_instructions (
  id SERIAL PRIMARY KEY,
  tax_date DATE NOT NULL,
  instruction_date DATE NOT NULL,
  source_account_code VARCHAR(64) NOT NULL DEFAULT 'chase_business_checking',
  destination_account_code VARCHAR(64) NOT NULL DEFAULT 'huntington_tax',
  transfer_method VARCHAR(64) NOT NULL DEFAULT 'chase_linked_external_transfer',
  amount_cents INTEGER NOT NULL CHECK (amount_cents >= 0),
  currency CHAR(3) NOT NULL DEFAULT 'USD' CHECK (currency = 'USD'),
  recommendation_source VARCHAR(64) NOT NULL DEFAULT 'copilot_collected_tax' CHECK (recommendation_source = 'copilot_collected_tax'),
  recommendation_as_of TIMESTAMP NOT NULL,
  tax_summary_snapshot_source VARCHAR(64) NOT NULL,
  backend_reconstructed_tax_cents INTEGER NOT NULL DEFAULT 0,
  variance_cents INTEGER NOT NULL DEFAULT 0,
  status VARCHAR(32) NOT NULL CHECK (status IN ('pending_approval', 'approved', 'submitted', 'superseded', 'canceled')),
  memo TEXT NOT NULL,
  generation_trigger VARCHAR(16) NOT NULL CHECK (generation_trigger IN ('cron', 'manual')),
  approved_at TIMESTAMP,
  approved_by_user_id INTEGER REFERENCES admin_users(id),
  approved_by_name VARCHAR(255),
  approved_by_email VARCHAR(255),
  submitted_at TIMESTAMP,
  submitted_by_user_id INTEGER REFERENCES admin_users(id),
  submitted_by_name VARCHAR(255),
  submitted_by_email VARCHAR(255),
  bank_confirmation_ref VARCHAR(120),
  submission_note TEXT,
  canceled_at TIMESTAMP,
  canceled_by_user_id INTEGER REFERENCES admin_users(id),
  canceled_by_name VARCHAR(255),
  canceled_by_email VARCHAR(255),
  cancellation_reason TEXT,
  superseded_at TIMESTAMP,
  superseded_by_instruction_id INTEGER REFERENCES tax_transfer_instructions(id),
...
const PROPERTY_MEASUREMENT_REQUESTS_TABLE = `CREATE TABLE IF NOT EXISTS property_measurement_requests (
  id SERIAL PRIMARY KEY,
  property_id INTEGER NOT NULL REFERENCES properties(id) ON DELETE CASCADE,
  requested_by_user_id INTEGER REFERENCES admin_users(id),
  requested_by_name VARCHAR(255),
  status VARCHAR(32) NOT NULL DEFAULT 'queued' CHECK (status IN ('queued', 'processing', 'completed', 'failed')),
  request_type VARCHAR(32) NOT NULL DEFAULT 'automeasure',
  report_type VARCHAR(64) NOT NULL DEFAULT 'landscaping',
  boundary_source VARCHAR(32) DEFAULT 'parcel',
...
const PROPERTY_MEASUREMENTS_TABLE = `CREATE TABLE IF NOT EXISTS property_measurements (
  id SERIAL PRIMARY KEY,
  property_id INTEGER NOT NULL REFERENCES properties(id) ON DELETE CASCADE,
  request_id INTEGER REFERENCES property_measurement_requests(id) ON DELETE SET NULL,
  version INTEGER NOT NULL DEFAULT 1,
  status VARCHAR(32) NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'completed', 'superseded')),
  method VARCHAR(120),
  lot_area_sqft INTEGER,
  confidence JSONB NOT NULL DEFAULT '{}'::jsonb,
  summary JSONB NOT NULL DEFAULT '{}'::jsonb,
  parcel_data JSONB NOT NULL DEFAULT '{}'::jsonb,
  imagery_data JSONB NOT NULL DEFAULT '{}'::jsonb,
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
)`;

const PROPERTY_MEASUREMENT_FEATURES_TABLE = `CREATE TABLE IF NOT EXISTS property_measurement_features (
  id SERIAL PRIMARY KEY,
  measurement_id INTEGER NOT NULL REFERENCES property_measurements(id) ON DELETE CASCADE,
  feature_key VARCHAR(120) NOT NULL,
  feature_label VARCHAR(255) NOT NULL,
  feature_group VARCHAR(32) NOT NULL CHECK (feature_group IN ('surface', 'edge', 'point')),
  geometry_type VARCHAR(32) NOT NULL CHECK (geometry_type IN ('polygon', 'line', 'point', 'derived')),
...
async function runStartupMigrations(pool) {
  ...
  const newTables = [
    ...
    COPILOT_TAX_SUMMARY_SNAPSHOTS_TABLE, TAX_TRANSFER_INSTRUCTIONS_TABLE,
    PROPERTY_MEASUREMENT_REQUESTS_TABLE, PROPERTY_MEASUREMENTS_TABLE, PROPERTY_MEASUREMENT_FEATURES_TABLE,
  ];
  ...
  try { await pool.query(`CREATE INDEX IF NOT EXISTS idx_tax_transfer_instructions_tax_date ON tax_transfer_instructions (tax_date DESC, created_at DESC)`); } catch(e) {}
  try { await pool.query(`CREATE INDEX IF NOT EXISTS idx_tax_transfer_instructions_status ON tax_transfer_instructions (status)`); } catch(e) {}
  try { await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uniq_tax_transfer_instructions_current_per_tax_date ON tax_transfer_instructions (tax_date) WHERE status IN ('pending_approval', 'approved', 'submitted')`); } catch(e) {}
  ...
  //     view_path, edit_path, raw_status, ...).
  ...
  const invoiceImportCols = [
    ...
    'ALTER TABLE invoices ADD COLUMN IF NOT EXISTS sent_status VARCHAR(40)',
  ...
  try {
    ...
    const quoteLeadColumns = [
      "ALTER TABLE quotes ADD COLUMN IF NOT EXISTS service_area_status VARCHAR(30) DEFAULT 'review'",
      'ALTER TABLE quotes ADD COLUMN IF NOT EXISTS service_area_reason TEXT',
      "ALTER TABLE quotes ADD COLUMN IF NOT EXISTS homeworks_sync_status VARCHAR(30) DEFAULT 'pending'",
      'ALTER TABLE quotes ADD COLUMN IF NOT EXISTS homeworks_customer_id BIGINT',
      'ALTER TABLE quotes ADD COLUMN IF NOT EXISTS homeworks_property_id BIGINT',
      'ALTER TABLE quotes ADD COLUMN IF NOT EXISTS homeworks_match_type VARCHAR(60)',
      'ALTER TABLE quotes ADD COLUMN IF NOT EXISTS homeworks_sync_error TEXT',
      'ALTER TABLE quotes ADD COLUMN IF NOT EXISTS homeworks_synced_at TIMESTAMP',
      'ALTER TABLE quotes ADD COLUMN IF NOT EXISTS contacted_at TIMESTAMP',
      'ALTER TABLE quotes ADD COLUMN IF NOT EXISTS follow_up_due_at TIMESTAMP',
      'ALTER TABLE quotes ADD COLUMN IF NOT EXISTS last_follow_up_at TIMESTAMP',
      'ALTER TABLE quotes ADD COLUMN IF NOT EXISTS assigned_to VARCHAR(100)',
      "ALTER TABLE quotes ADD COLUMN IF NOT EXISTS tim_handoff_status VARCHAR(30) DEFAULT 'not_sent'",
      'ALTER TABLE quotes ADD COLUMN IF NOT EXISTS tim_handoff_at TIMESTAMP',
      "ALTER TABLE quotes ADD COLUMN IF NOT EXISTS photos JSONB DEFAULT '[]'::jsonb",
      'ALTER TABLE quotes ADD COLUMN IF NOT EXISTS submission_fingerprint VARCHAR(64)',
      'CREATE INDEX IF NOT EXISTS quotes_follow_up_due_idx ON quotes(follow_up_due_at) WHERE status NOT IN (\'completed\', \'cancelled\')',
      'CREATE INDEX IF NOT EXISTS quotes_homeworks_sync_idx ON quotes(homeworks_sync_status)',
      'CREATE UNIQUE INDEX IF NOT EXISTS quotes_submission_fingerprint_uidx ON quotes(submission_fingerprint) WHERE submission_fingerprint IS NOT NULL',
    ];
    for (const sql of quoteLeadColumns) await pool.query(sql);
    await pool.query(`CREATE TABLE IF NOT EXISTS quote_lead_events (
    ...
    await pool.query('CREATE INDEX IF NOT EXISTS quote_lead_events_quote_idx ON quote_lead_events(quote_id, created_at DESC)');
  ...
  // Yard sign responses table
  try {
    await pool.query(`CREATE TABLE IF NOT EXISTS yard_sign_responses (
      id SERIAL PRIMARY KEY,
  ...
  try {
    ...
    await pool.query(`ALTER TABLE properties ADD COLUMN IF NOT EXISTS latitude DECIMAL(10,7)`).catch(() => {});
    await pool.query(`ALTER TABLE properties ADD COLUMN IF NOT EXISTS longitude DECIMAL(10,7)`).catch(() => {});
    await pool.query(`ALTER TABLE properties ADD COLUMN IF NOT EXISTS parcel_id VARCHAR(120)`).catch(() => {});
    await pool.query(`ALTER TABLE properties ADD COLUMN IF NOT EXISTS parcel_data JSONB DEFAULT '{}'::jsonb`).catch(() => {});
    await pool.query(`ALTER TABLE properties ADD COLUMN IF NOT EXISTS parcel_boundary_geojson JSONB`).catch(() => {});
    await pool.query(`ALTER TABLE properties ADD COLUMN IF NOT EXISTS measurement_summary JSONB DEFAULT '{}'::jsonb`).catch(() => {});
    await pool.query(`ALTER TABLE properties ADD COLUMN IF NOT EXISTS last_measured_at TIMESTAMP`).catch(() => {});
  ...
  try {
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS started_at TIMESTAMP`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS started_by VARCHAR(255)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS completed_by VARCHAR(255)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS completion_lat DECIMAL(10,7)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS completion_lng DECIMAL(10,7)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS last_status_at TIMESTAMP`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS last_status_by VARCHAR(255)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS last_status_source VARCHAR(30) DEFAULT 'legacy'`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS dispatch_issue BOOLEAN DEFAULT false`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS dispatch_issue_reason TEXT`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS dispatch_issue_reported_at TIMESTAMP`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS dispatch_issue_reported_by VARCHAR(255)`).catch(() => {});
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_scheduled_jobs_dispatch_board ON scheduled_jobs (job_date, crew_assigned, route_order, status)`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_scheduled_jobs_dispatch_issue ON scheduled_jobs (job_date, dispatch_issue) WHERE dispatch_issue = true`).catch(() => {});
    await pool.query(`
      UPDATE scheduled_jobs
      SET last_status_at = COALESCE(completed_at, updated_at, created_at),
          last_status_source = COALESCE(last_status_source, 'legacy')
      WHERE last_status_at IS NULL OR last_status_source IS NULL
    `).catch(() => {});
    console.log('✅ Job completion columns ready');
  } catch(e) { console.error('Job completion columns error:', e.message); }

  // Add Copilot execution mirror columns to scheduled_jobs if missing
  try {
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_job_id VARCHAR(100)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_visit_id VARCHAR(100)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_assigned_crew_name VARCHAR(255)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_execution_status VARCHAR(30)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_execution_status_raw VARCHAR(100)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_execution_reason TEXT`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_started_at TIMESTAMP`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_started_by VARCHAR(255)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_completed_at TIMESTAMP`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_completed_by VARCHAR(255)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_completion_notes TEXT`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_completion_photos JSONB DEFAULT '[]'::jsonb`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_completion_lat DECIMAL(10,7)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_completion_lng DECIMAL(10,7)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_event_updated_at TIMESTAMP`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_last_synced_at TIMESTAMP`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_payload_hash VARCHAR(64)`).catch(() => {});
    await pool.query(`ALTER TABLE scheduled_jobs ADD COLUMN IF NOT EXISTS copilot_execution_locked BOOLEAN DEFAULT false`).catch(() => {});
    await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_scheduled_jobs_copilot_visit_id ON scheduled_jobs (copilot_visit_id) WHERE copilot_visit_id IS NOT NULL`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_scheduled_jobs_copilot_job_date ON scheduled_jobs (copilot_job_id, job_date) WHERE copilot_job_id IS NOT NULL`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_scheduled_jobs_copilot_execution_status ON scheduled_jobs (copilot_execution_status, job_date) WHERE copilot_execution_status IS NOT NULL`).catch(() => {});
    console.log('✅ Copilot dispatch execution mirror columns ready');
  } catch(e) { console.error('Copilot dispatch execution mirror columns error:', e.message); }
}
...
async function ensureQBTables(pool) {
  ...
  try { await pool.query(`ALTER TABLE payments ADD COLUMN IF NOT EXISTS customer_name VARCHAR(255)`); } catch(e) {}
  try { await pool.query(`ALTER TABLE payments ADD COLUMN IF NOT EXISTS tip_amount DECIMAL(10,2) DEFAULT 0`); } catch(e) {}
  try { await pool.query(`ALTER TABLE payments ADD COLUMN IF NOT EXISTS details TEXT`); } catch(e) {}
  try { await pool.query(`ALTER TABLE payments ADD COLUMN IF NOT EXISTS source_date_raw VARCHAR(255)`); } catch(e) {}
  try { await pool.query(`ALTER TABLE payments ADD COLUMN IF NOT EXISTS external_source VARCHAR(40)`); } catch(e) {}
  try { await pool.query(`ALTER TABLE payments ADD COLUMN IF NOT EXISTS external_payment_key VARCHAR(120)`); } catch(e) {}
  try { await pool.query(`ALTER TABLE payments ADD COLUMN IF NOT EXISTS external_metadata JSONB DEFAULT '{}'::jsonb`); } catch(e) {}
  try { await pool.query(`ALTER TABLE payments ADD COLUMN IF NOT EXISTS imported_at TIMESTAMP`); } catch(e) {}
  try {
    await pool.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS payments_external_uidx
      ON payments(external_source, external_payment_key)
...
async function ensureCopilotLiveTables(pool) {
  await pool.query(`CREATE TABLE IF NOT EXISTS copilot_live_jobs (
    job_key TEXT PRIMARY KEY,
    source_system TEXT NOT NULL DEFAULT 'copilot' CHECK (source_system = 'copilot'),
    ...
    job_title TEXT,
    source_status TEXT,
    visit_total NUMERIC(10,2),
    source_crew_name TEXT,
    source_employees_text TEXT,
    source_stop_order INTEGER,
    address_raw TEXT,
    ...
    UNIQUE (source_system, service_date, source_event_id)
  )`);
  await pool.query(`CREATE INDEX IF NOT EXISTS copilot_live_jobs_date_idx ON copilot_live_jobs(service_date) WHERE source_deleted_at IS NULL`);
  await pool.query(`CREATE INDEX IF NOT EXISTS copilot_live_jobs_crew_idx ON copilot_live_jobs(service_date, source_crew_name) WHERE source_deleted_at IS NULL`);

  await pool.query(`CREATE TABLE IF NOT EXISTS yarddesk_job_overlays (
    job_key TEXT PRIMARY KEY REFERENCES copilot_live_jobs(job_key) ON DELETE CASCADE,
    review_state TEXT NOT NULL DEFAULT 'new' CHECK (review_state IN ('new', 'reviewed', 'needs_attention')),
    office_note TEXT,
    hold_from_dispatch BOOLEAN NOT NULL DEFAULT false,
    local_tags JSONB NOT NULL DEFAULT '[]'::jsonb CHECK (jsonb_typeof(local_tags) = 'array'),
    address_override TEXT,
    customer_link_id INTEGER REFERENCES customers(id) ON DELETE SET NULL,
    property_link_id INTEGER REFERENCES properties(id) ON DELETE SET NULL,
    updated_by_user_id INTEGER,
    updated_by_name TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  )`);
  await pool.query(`CREATE INDEX IF NOT EXISTS yarddesk_job_overlays_review_idx ON yarddesk_job_overlays(review_state, hold_from_dispatch)`);

  await pool.query(`CREATE TABLE IF NOT EXISTS dispatch_plan_items (
    job_key TEXT PRIMARY KEY REFERENCES copilot_live_jobs(job_key) ON DELETE CASCADE,
    service_date DATE NOT NULL,
    crew_override_name TEXT,
    route_order_override INTEGER CHECK (route_order_override IS NULL OR route_order_override > 0),
    route_locked BOOLEAN NOT NULL DEFAULT false,
    map_lat NUMERIC(10,7),
    map_lng NUMERIC(10,7),
    map_source TEXT NOT NULL DEFAULT 'none' CHECK (map_source IN ('none', 'geocoded', 'manual_override')),
    map_quality TEXT NOT NULL DEFAULT 'missing' CHECK (map_quality IN ('street', 'ambiguous', 'missing', 'failed')),
    map_address_input TEXT,
    map_updated_at TIMESTAMPTZ,
    print_group_key TEXT,
    print_note TEXT,
    updated_by_user_id INTEGER,
    updated_by_name TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CHECK ((map_lat IS NULL AND map_lng IS NULL) OR (map_lat IS NOT NULL AND map_lng IS NOT NULL))
  )`);
  await pool.query(`CREATE INDEX IF NOT EXISTS dispatch_plan_items_date_idx ON dispatch_plan_items(service_date)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS dispatch_plan_items_crew_route_idx ON dispatch_plan_items(service_date, crew_override_name, route_order_override)`);

  await pool.query(`CREATE TABLE IF NOT EXISTS dispatch_route_templates (
    id SERIAL PRIMARY KEY,
    seed_key TEXT,
    name TEXT NOT NULL,
  ...
  await pool.query(`ALTER TABLE dispatch_route_templates ADD COLUMN IF NOT EXISTS seed_key TEXT`);
  await pool.query(`CREATE INDEX IF NOT EXISTS dispatch_route_templates_active_idx ON dispatch_route_templates(active, crew_name, day_of_week)`);
  await pool.query(`DROP INDEX IF EXISTS dispatch_route_templates_seed_key_uidx`);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS dispatch_route_templates_seed_key_uidx ON dispatch_route_templates(seed_key)`);

  await pool.query(`CREATE TABLE IF NOT EXISTS dispatch_route_template_stops (
    id SERIAL PRIMARY KEY,
    template_id INTEGER NOT NULL REFERENCES dispatch_route_templates(id) ON DELETE CASCADE,
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS dispatch_route_template_stops_template_idx ON dispatch_route_template_stops(template_id, position)`);
}
...
async function runStartupTableInit(pool) {
  try {
    await pool.query(`CREATE TABLE IF NOT EXISTS site_chats (
      id UUID PRIMARY KEY,
      visitor_token_hash CHAR(64) NOT NULL,
      visitor_name VARCHAR(80) NOT NULL,
      visitor_contact VARCHAR(160),
      mode VARCHAR(12) NOT NULL DEFAULT 'human' CHECK (mode IN ('assistant', 'human')),
      alerted_at TIMESTAMPTZ,
      status VARCHAR(12) NOT NULL DEFAULT 'open',
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )`);
    await pool.query(`CREATE INDEX IF NOT EXISTS site_chats_updated_idx ON site_chats (updated_at DESC)`);
    await pool.query(`ALTER TABLE site_chats ADD COLUMN IF NOT EXISTS joined_by VARCHAR(80)`);
    await pool.query(`ALTER TABLE site_chats ADD COLUMN IF NOT EXISTS joined_at TIMESTAMPTZ`);
    await pool.query(`CREATE TABLE IF NOT EXISTS site_chat_messages (
      id BIGSERIAL PRIMARY KEY,
      chat_id UUID NOT NULL REFERENCES site_chats(id) ON DELETE CASCADE,
      sender VARCHAR(12) NOT NULL CHECK (sender IN ('visitor', 'assistant', 'staff')),
      body VARCHAR(1200) NOT NULL,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )`);
    await pool.query(`ALTER TABLE site_chat_messages ADD COLUMN IF NOT EXISTS staff_name VARCHAR(80)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS site_chat_messages_chat_idx ON site_chat_messages (chat_id, id)`);
    await pool.query(`CREATE TABLE IF NOT EXISTS site_chat_weekly_reports (
      week_start DATE PRIMARY KEY,
      status VARCHAR(12) NOT NULL CHECK (status IN ('sending', 'sent', 'failed')),
    ...
    await ensureCopilotLiveTables(pool);
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS app_ai_insights (
      id SERIAL PRIMARY KEY,
      ...
      service_area_status VARCHAR(20) NOT NULL DEFAULT 'unknown',
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS app_ai_insights_phone_idx ON app_ai_insights(phone_number, analyzed_at DESC)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS app_ai_insights_priority_idx ON app_ai_insights(is_quote_request, urgency, analyzed_at DESC)`);
    await pool.query(`CREATE TABLE IF NOT EXISTS app_ai_tasks (
      id SERIAL PRIMARY KEY,
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS app_ai_tasks_status_due_idx ON app_ai_tasks(status, due_at, priority)`);
