...
function invoiceBalance(invoice) {
  if (invoice.is_sent === false || invoice.is_archived) return 0;
  if (['paid', 'draft', 'void', 'write_off', 'write off'].includes(safeText(invoice.status).toLowerCase())) return 0;
  // Metadata total_due may be account-wide, not this invoice's balance.
...
function buildPaymentActivity(invoices, payments, activityFrom, activityTo) {
  const activity = [...payments];
  const referencedInvoices = new Set();
  activity.forEach((payment) => {
    const direct = safeText(payment.invoice_number);
    if (direct) referencedInvoices.add(direct.toLowerCase());
    const text = [payment.details, payment.notes, parseJson(payment.external_metadata).raw_details]
      .map(safeText)
      .filter(Boolean)
      .join(' ');
    for (const match of text.matchAll(/invoice\s*#\s*([a-z0-9-]+)/gi)) {
      referencedInvoices.add(match[1].toLowerCase());
    }
  });

  const from = activityFrom ? new Date(`${String(activityFrom).slice(0, 10)}T00:00:00Z`) : null;
  const to = activityTo ? new Date(`${String(activityTo).slice(0, 10)}T23:59:59Z`) : null;
  invoices.forEach((invoice) => {
    ...
    if (paid <= 0.004 || !paidAt || Number.isNaN(paidAt.getTime()) || !invoiceNumber) return;
    if (from && paidAt < from) return;
    if (to && paidAt > to) return;
    if (referencedInvoices.has(invoiceNumber.toLowerCase())) return;

    const metadata = parseJson(invoice.external_metadata || invoice.metadata);
    activity.push({
      paid_at: invoice.paid_at,
      amount: paid,
      method: safeText(metadata.payment_method || invoice.payment_method || 'Payment recorded'),
      invoice_number: invoiceNumber,
      details: `Applied to Invoice #${invoiceNumber}`,
      derived_from_invoice: true,
    });
  });

  return activity.sort((a, b) => new Date(b.paid_at || b.created_at || 0) - new Date(a.paid_at || a.created_at || 0));
}
...
function drawInvoiceRow(page, ctx, y, invoice, statementDate) {
  ...
  const isPastDue = balance > 0 && (typeof invoice.is_past_due === 'boolean' ? invoice.is_past_due : dueAt && !Number.isNaN(dueAt.getTime()) && dueAt < statementAt);
...
async function renderStatementPdf({
  ...
  const activity = buildPaymentActivity(invoices, payments, activityFrom, activityTo || statementDate);
  const statementAt = new Date(statementDate);
  const totals = openInvoices.reduce((summary, invoice) => {
    const balance = invoiceBalance(invoice);
    const due = invoice.due_date ? new Date(invoice.due_date) : null;
    const status = safeText(invoice.status).toLowerCase();
    const isPastDue = typeof invoice.is_past_due === 'boolean' ? invoice.is_past_due
      : status.includes('overdue') || status.includes('past due') || status === 'past_due' || (due && !Number.isNaN(due.getTime()) && due < statementAt);
  ...
  const calcY = y - 15;
  const appliedAmount = totals.applied > 0.004 ? `-${money(totals.applied)}` : money(0);
  ...
  rightText(page, appliedAmount, PAGE_WIDTH - MARGIN - 14, calcY - 29, { size: 8.5, font: bold, color: COLORS.ink });
  ...
  if (activity.length) {
    if (y < 145) newPage();
    y = drawSectionTitle(page, ctx, y, 'PAYMENT ACTIVITY', 'Recorded payments and account credits; these are already reflected in invoice balances.');
    y = drawPaymentHeader(page, ctx, y);
    for (let index = 0; index < activity.length; index += 1) {
      if (y < 88) { newPage(); y = drawSectionTitle(page, ctx, y, 'PAYMENT ACTIVITY', 'Continued'); y = drawPaymentHeader(page, ctx, y); }
