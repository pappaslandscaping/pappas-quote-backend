function resolveMailAccountSummary({
  ...
  let priorBalance = 0;
  if (customerOutstandingBalance !== null && customerOutstandingBalance > 0) {
    if (Math.abs(customerOutstandingBalance - normalizedInvoiceTotal) <= 0.02) {
      priorBalance = 0;
    } else if (customerOutstandingBalance > normalizedInvoiceTotal + 0.009) {
      priorBalance = roundMoney(customerOutstandingBalance - normalizedInvoiceTotal);
    } else {
  ...
  return {
    priorBalance,
    thisInvoice: normalizedInvoiceTotal,
    totalDueOnAccount: roundMoney(normalizedInvoiceTotal + priorBalance),
  };
}
