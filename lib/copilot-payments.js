...
function extractInvoiceDateFromDetails(value) {
  const text = cleanText(value);
  if (!text) return null;
  const match = text.match(/invoice\s*#\s*[a-z0-9-]+\s+([A-Za-z]{3,9}\s+\d{1,2},\s+\d{4})/i);
  return match ? parseMonthDayYear(match[1]) : null;
}
...
function parseCopilotPaymentsHtml(html, pageUrl) {
  ...
  if (!table) {
    return {
      ...
      parser_warning: 'Payments table not found',
    };
  }
  ...
  bodyRows.forEach((row) => {
    ...
    const extractedInvoiceDate = extractInvoiceDateFromDetails(details);
    ...
    rows.push({
      ...
      extracted_invoice_date: extractedInvoiceDate || null,
...
function parseCopilotPaymentDetailHtml(html) {
  const $ = cheerio.load(html || '');
  const text = cleanText($('body').text());

  // Prefer the structured label/value row. Cheerio can concatenate adjacent
  // table cells without whitespace, which made "Payment Method" + "Check #358"
  // invisible to the old body-text regex.
  let method = '';
  $('tr').each((_, row) => {
    if (method) return;
    const cells = $(row).find('th,td').toArray();
    if (cells.length < 2) return;
    if (normalizeHeader($(cells[0]).text()) === 'payment method') {
      method = cleanText($(cells[1]).text());
    }
  });

  if (!method) {
    const methodMatch = text.match(/Payment\s*Method\s*(.+?)(?=(?:\s+)?(?:Payer|Tip|Details|Payment\s*Notes|$))/i);
    method = cleanText(methodMatch?.[1]);
  }

  const checkMatch = method.match(/check\s*#\s*([a-z0-9-]+)/i)
    || text.match(/Payment\s*Method\s*Check\s*#\s*([a-z0-9-]+)/i);
  return {
    method: method || null,
...
function normalizeCopilotPaymentsSnapshot(snapshot, sourceOverride = LIVE_COPILOT_SOURCE) {
  ...
  const payments = snapshot.payments.map((payment) => ({
    ...
    extracted_invoice_date: cleanText(payment.extracted_invoice_date) || null,
    external_source: 'copilotcrm',
