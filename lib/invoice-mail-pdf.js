...
function toSummary(invoice) {
  ...
  return {
    ...
    lateFee: parseMoney(metadata.late_fee) ?? 0,
    processingFee: parseMoney(metadata.processing_fee) ?? 0,
    discountAmount: parseMoney(metadata.discount_amount) ?? 0,
...
function drawSummaryAndNotes(page, invoice, fonts, yTop) {
  ...
  const optionalRows = [summary.lateFee, summary.processingFee, summary.discountAmount, summary.paymentCredit, summary.priorBalance].filter(value => value > 0).length;
  const blockHeight = Math.max(leftHeight, 48 + ((3 + optionalRows) * 22));
  ...
  const totalsGap = optionalRows >= 2 ? 22 : 26;
  ...
  for (const [label, amount, credit] of [
    ['Discount', summary.discountAmount, true],
    ['Late Fee', summary.lateFee, false],
    ['Processing Fee', summary.processingFee, false],
  ]) {
    if (amount <= 0) continue;
    drawKeyValue(page, label, `${credit ? '-' : ''}${money(amount)}`, rightX + 16, totalsY, rightWidth - 32, fonts);
    totalsY -= totalsGap;
  }
