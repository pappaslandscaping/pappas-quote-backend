...
async function ensureCopilotLiveTables(pool) {
  await pool.query(`CREATE TABLE IF NOT EXISTS copilot_live_jobs (
    ...
    source_surface TEXT NOT NULL DEFAULT 'route_day' CHECK (source_surface IN ('route_day', 'schedule_grid')),
  ...
  try { await pool.query(`ALTER TABLE copilot_live_jobs ADD COLUMN IF NOT EXISTS source_surface TEXT`); } catch (e) {}
  try { await pool.query(`UPDATE copilot_live_jobs SET source_surface = 'route_day' WHERE source_surface IS NULL`); } catch (e) {}
  try { await pool.query(`ALTER TABLE copilot_live_jobs ALTER COLUMN source_surface SET DEFAULT 'route_day'`); } catch (e) {}
  try { await pool.query(`ALTER TABLE copilot_live_jobs ALTER COLUMN source_surface SET NOT NULL`); } catch (e) {}
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS copilot_live_jobs_surface_date_idx ON copilot_live_jobs(source_surface, service_date) WHERE source_deleted_at IS NULL`);

  await pool.query(`CREATE TABLE IF NOT EXISTS copilot_schedule_live_jobs (
    job_key TEXT PRIMARY KEY,
    source_system TEXT NOT NULL DEFAULT 'copilot' CHECK (source_system = 'copilot'),
    source_surface TEXT NOT NULL DEFAULT 'schedule_grid' CHECK (source_surface = 'schedule_grid'),
    service_date DATE NOT NULL,
    source_event_id TEXT NOT NULL,
    source_customer_id TEXT,
    customer_name TEXT NOT NULL,
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS copilot_schedule_live_jobs_date_idx ON copilot_schedule_live_jobs(service_date) WHERE source_deleted_at IS NULL`);
  await pool.query(`CREATE INDEX IF NOT EXISTS copilot_schedule_live_jobs_crew_idx ON copilot_schedule_live_jobs(service_date, source_crew_name) WHERE source_deleted_at IS NULL`);
  await pool.query(`CREATE INDEX IF NOT EXISTS copilot_schedule_live_jobs_surface_date_idx ON copilot_schedule_live_jobs(source_surface, service_date) WHERE source_deleted_at IS NULL`);
