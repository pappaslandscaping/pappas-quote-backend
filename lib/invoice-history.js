...
function buildInvoiceHistoryEvents(invoice, options = {}) {
  ...
  const hasSentEmailEvent = emailEvents.some(event => event.type === 'sent' || event.badge === 'Sent by Email');
  const hasReminderEmailEvent = emailEvents.some(event => event.type === 'reminder' || event.badge === 'Reminder Sent');

  const sentAt = parseEventDate(invoice.sent_at);
  if (sentAt && !hasSentEmailEvent) {
    events.push({
      type: 'sent',
      date: sentAt,
      badge: 'Sent',
  ...
  if (reminderSentAt && !hasReminderEmailEvent) {
    events.push({
      type: 'reminder',
      date: reminderSentAt,
      badge: 'Reminder Sent',
