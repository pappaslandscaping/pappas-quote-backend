...
function compactNoteText(notes) {
  const cleaned = stripMarkup(notes);
  if (!cleaned) {
    return 'Please return the remittance stub with your mailed check and include the invoice number on payment.';
  }
  ...
  for (const sentence of sentences) {
    if (short.join(' ').length + sentence.length > 160) break;
    short.push(sentence);
    if (short.length >= 1) break;
  }

  const result = short.join(' ').trim() || cleaned.slice(0, 160).trim();
  return result.length > 160 ? `${result.slice(0, 157).trimEnd()}...` : result;
}
    ...
    <section class="page page-break">
      ...
      <div class="bottom">
        ...
        <div class="totals-box">
          <div class="totals-row"><span>Subtotal</span><span>${money(invoice.subtotal || 0)}</span></div>
          <div class="totals-row"><span>Taxes</span><span>${money(invoice.tax_amount || 0)}</span></div>
      ...
      <div class="stub">
        ...
        <div class="stub-mid">
          <div class="stub-panel">
            ...
            <strong>${escapeHtml(COMPANY.name)}</strong>
            ${COMPANY.remitLines.slice(0, 2).map((line) => `<div>${escapeHtml(line)}</div>`).join('')}
          </div>
        ...
        <div class="stub-bottom">
          ...
          <div class="amount-due">
            ...
            <div class="value">${money(summary.totalDueOnAccount)}</div>
          </div>
        </div>
      </div>
