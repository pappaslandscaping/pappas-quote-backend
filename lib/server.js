...
const { appendHtmlBeforeBodyEnd, renderEmailMarkup } = require('./lib/email-renderer');
...
const {
  syncPaymentsCsvToDatabase,
  calculatePaymentTaxPortion,
...
function buildStoredEmailRenderOptions(options = {}) {
  const baseUrl = process.env.BASE_URL || 'https://app.pappaslandscaping.com';

  return {
    wrapper: options.wrapper || 'full',
    format: options.format,
    showSignature: options.showSignature,
    showFeatures: options.showFeatures,
    htmlWrapper: emailTemplate,
    logoUrl: LOGO_URL,
    companyName: COMPANY_NAME,
    signatureImage: SIGNATURE_IMAGE,
    footerPhoneDisplay: '(440) 886-7318',
    footerPhoneHref: '4408867318',
    footerAddress: 'PO Box 770057 • Lakewood, Ohio 44107',
    footerWebsiteUrl: 'https://pappaslandscaping.com',
    footerWebsiteLabel: 'pappaslandscaping.com',
    baseUrl,
    socialLinks: [
      { label: 'Facebook', href: 'https://www.facebook.com/pappaslandscaping' },
      { label: 'Instagram', href: 'https://www.instagram.com/pappaslandscaping' },
      { label: 'Nextdoor', href: 'https://nextdoor.com/profile/01ZjZkwxhPWdnML2k' },
    ],
    mjmlOptions: {
      filePath: __dirname,
      ...(options.mjmlOptions || {}),
...
async function renderStoredEmailHtml(content, options = {}) {
  return renderEmailMarkup(content, buildStoredEmailRenderOptions(options));
}

function applyUnsubscribePlaceholder(html, unsubscribeValue = '') {
  if (!html || !html.includes('{unsubscribe_email}')) {
    return html;
  }
  return html.replace(/\{unsubscribe_email\}/g, unsubscribeValue);
}
...
// POST /api/import-copilot-payments - Import Copilot payments CSV and compute tax sweep totals
app.post('/api/import-copilot-payments', upload.single('csvfile'), async (req, res) => {
  try {
    ...
    const csvContent = req.file.buffer.toString('utf-8');
    const result = await syncPaymentsCsvToDatabase(pool, csvContent);
    ...
    console.error('Copilot payments import error:', error);
    serverError(res, error);
  }
});
...
<div class="footer">
  <div class="accent-bar" style="margin:15px 0;"></div>
    ...
    const companyPhone = '(440) 886-7318';
    const companyEmail = 'hello@pappaslandscaping.com';
    ...
    if (updatedQuote.customer_email) {
      ...
      const customerContent = `
        ...
        <div style="background:#f8faf6;border-radius:12px;padding:20px 24px;margin:0 0 24px;">
          <p style="margin:0 0 12px;color:#2e403d;font-size:16px;font-weight:700;">Your customer portal is ready</p>
          <p style="font-size:15px;color:#4a5568;line-height:1.8;margin:0 0 16px;">To complete your setup, please add a card or bank account on file in your portal.</p>
          <p style="font-size:15px;color:#4a5568;line-height:1.8;margin:0 0 20px;">You can also use the portal to view invoices, estimates, payment history, and account details in one place.</p>
          <p style="margin:0 0 12px;color:#2e403d;font-size:16px;font-weight:700;">Inside Your Portal</p>
          ...
          <div style="text-align:center;margin:0 0 24px;">
            <a href="${portalUrl}" style="display:inline-block;padding:14px 40px;background:#2e403d;color:#ffffff;text-decoration:none;border-radius:8px;font-size:16px;font-weight:600;">Open Customer Portal</a>
          </div>
          <p style="font-size:15px;color:#4a5568;line-height:1.8;margin:0;">Questions? Call ${companyPhone} or email ${companyEmail} and we’ll be happy to help.</p>
        </div>
      `;
...
app.get('/api/payments', async (req, res) => {
  try {
    ...
    if (search) {
      where.push(`(COALESCE(i.customer_name, '') ILIKE $${p} OR COALESCE(i.invoice_number, '') ILIKE $${p} OR COALESCE(p.payment_id, '') ILIKE $${p})`);
      params.push('%' + search + '%'); p++;
    }
    if (year) {
      where.push(`EXTRACT(YEAR FROM COALESCE(p.paid_at, p.updated_at)) = $${p}`);
      params.push(parseInt(year)); p++;
    }
    if (month) {
      where.push(`EXTRACT(MONTH FROM COALESCE(p.paid_at, p.updated_at)) = $${p}`);
    ...
    const [result, countResult, monthly] = await Promise.all([
      pool.query(
        `SELECT
            ...
            i.payment_token,
            CASE
              WHEN COALESCE(i.total, 0) > 0 AND COALESCE(i.tax_amount, 0) > 0
                THEN ROUND((p.amount * i.tax_amount / i.total)::numeric, 2)
              ELSE 0
            END AS tax_portion_collected
         FROM payments p
         LEFT JOIN invoices i ON i.id = p.invoice_id
         ${whereClause}
         ORDER BY COALESCE(p.paid_at, p.updated_at) DESC, p.id DESC
         LIMIT $${p} OFFSET $${p+1}`,
        params
      ),
      pool.query(
        `SELECT COUNT(*) as cnt, COALESCE(SUM(p.amount),0) as total_received, COALESCE(SUM(
            CASE
              WHEN COALESCE(i.total, 0) > 0 AND COALESCE(i.tax_amount, 0) > 0
                THEN ROUND((p.amount * i.tax_amount / i.total)::numeric, 2)
              ELSE 0
            END
          ),0) as total_tax_received
         FROM payments p
         LEFT JOIN invoices i ON i.id = p.invoice_id
         ${whereClause}`,
        params.slice(0, -2)
      ...
      pool.query(`
        SELECT
               to_char(COALESCE(p.paid_at, p.updated_at),'YYYY-MM') as month,
               COUNT(*) as count,
        ...
        FROM payments p
        LEFT JOIN invoices i ON i.id = p.invoice_id
        WHERE p.status = 'completed' AND COALESCE(p.paid_at, p.updated_at) >= NOW() - INTERVAL '12 months'
    ...
    res.json({
      ...
      totalTaxReceived: parseFloat(countResult.rows[0].total_tax_received),
...
// GET /api/reports/tax-sweep - Recommended transfer amount based on actual completed payments
app.get('/api/reports/tax-sweep', async (req, res) => {
  try {
    await ensurePaymentsTables();
    await ensureInvoicesTable();

    const today = new Date().toISOString().slice(0, 10);
    const { start_date = today, end_date = today, method, source = 'all' } = req.query;
    ...
    if (source !== 'all') {
      where.push(`COALESCE(p.external_source, 'internal') = $${paramIndex}`);
      params.push(source);
      paramIndex += 1;
    }

    if (methods.length) {
      where.push(`LOWER(COALESCE(p.method, '')) = ANY($${paramIndex})`);
      params.push(methods);
      paramIndex += 1;
    }
    ...
    const payments = result.rows.map((row) => {
      ...
      const invoiceTaxAmount = parseFloat(row.tax_amount) || 0;
      const invoiceTotal = parseFloat(row.total) || 0;
      const taxPortion = calculatePaymentTaxPortion(
        Math.min(amount, invoiceTotal || amount),
        invoiceTaxAmount,
        invoiceTotal
      );
      ...
      return {
        ...
        method: row.method,
        paid_at: row.paid_at,
        source: row.external_source || 'internal',
    ...
    const recommendedTransferAmount = payments.reduce((sum, payment) => sum + payment.tax_portion, 0);
    ...
    for (const payment of payments) {
      ...
      byMethod[key].payment_count += 1;
      byMethod[key].payment_total += payment.amount;
      byMethod[key].tax_portion += payment.tax_portion;
    }
    ...
    res.json({
      ...
      recommended_transfer_amount: Number(recommendedTransferAmount.toFixed(2)),
      by_method: Object.values(byMethod).map((entry) => ({
        ...entry,
        payment_total: Number(entry.payment_total.toFixed(2)),
        tax_portion: Number(entry.tax_portion.toFixed(2)),
      })),
      payments,
    });
  } catch (error) {
...
async function renderTemplate(slug, vars, fallbackSubject, fallbackHtml) {
  ...
  if (template) {
    ...
    let html = await renderStoredEmailHtml(body, {
      wrapper: template.options?.wrapper || 'full',
      format: template.options?.format,
      showSignature: template.options?.showSignature,
      showFeatures: template.options?.showFeatures,
    });
    html = applyUnsubscribePlaceholder(html, vars.unsubscribe_email || (vars.customer_email ? encodeURIComponent(vars.customer_email) : ''));
...
const DEFAULT_TEMPLATES = [
  ...
  { name: 'Welcome Email', slug: 'welcome_email', category: 'marketing', subject: 'Welcome to Pappas & Co. Landscaping!', body: '<div style="font-size:12px;font-weight:700;letter-spacing:0.8px;text-transform:uppercase;color:#6b8f47;margin:0 0 12px;">Welcome</div><h2 style="color:#2e403d;font-size:28px;font-weight:700;line-height:1.2;margin:0 0 18px;">Your customer portal is ready</h2><p style="color:#374151;font-size:15px;line-height:1.8;margin:0 0 16px;">Hi {customer_first_name},</p><p style="color:#374151;font-size:15px;line-height:1.8;margin:0 0 16px;">Welcome to Pappas &amp; Co. Landscaping. Your customer portal is ready to use anytime.</p><p style="color:#374151;font-size:15px;line-height:1.8;margin:0 0 16px;">To complete your setup, please add a card or bank account on file in your portal.</p><p style="color:#374151;font-size:15px;line-height:1.8;margin:0 0 20px;">You can also use the portal to view invoices, estimates, payment history, and account details in one place.</p><div style="background:#f8faf6;border-radius:12px;padding:20px 24px;margin:0 0 24px;"><p style="margin:0 0 12px;color:#2e403d;font-size:16px;font-weight:700;">Inside Your Portal</p><table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="border-collapse:collapse;"><tr><td style="padding:12px 0;border-bottom:1px solid #d7dfd1;font-size:14px;font-weight:700;color:#1f2933;vertical-align:top;">Payment Method</td><td style="padding:12px 0 12px 18px;border-bottom:1px solid #d7dfd1;font-size:14px;line-height:1.6;color:#425466;vertical-align:top;">Add a card or bank account on file to complete your setup</td></tr><tr><td style="padding:12px 0;border-bottom:1px solid #d7dfd1;font-size:14px;font-weight:700;color:#1f2933;vertical-align:top;">Invoices</td><td style="padding:12px 0 12px 18px;border-bottom:1px solid #d7dfd1;font-size:14px;line-height:1.6;color:#425466;vertical-align:top;">View balances and payment activity</td></tr><tr><td style="padding:12px 0;border-bottom:1px solid #d7dfd1;font-size:14px;font-weight:700;color:#1f2933;vertical-align:top;">Estimates</td><td style="padding:12px 0 12px 18px;border-bottom:1px solid #d7dfd1;font-size:14px;line-height:1.6;color:#425466;vertical-align:top;">Review estimates and approvals</td></tr><tr><td style="padding:12px 0;font-size:14px;font-weight:700;color:#1f2933;vertical-align:top;">Work Requests</td><td style="padding:12px 0 12px 18px;font-size:14px;line-height:1.6;color:#425466;vertical-align:top;">Submit a new request anytime</td></tr></table></div><div style="text-align:center;margin:28px 0;"><a href="{portal_link}" style="background:#2e403d;color:#c9dd80;padding:14px 36px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;display:inline-block;">Open Customer Portal</a></div><p style="color:#374151;font-size:15px;line-height:1.8;margin:0;">Questions? Call {company_phone} or email {company_email} and we&rsquo;ll be happy to help.</p>', sms_body: 'Welcome to Pappas & Co., {customer_first_name}! We\'re excited to serve you. Questions? Call (440) 886-7318.', variables: '["customer_first_name","customer_name","portal_link","company_phone","company_email"]' },
...
app.post('/api/templates/send-preview', async (req, res) => {
  try {
    const { template_id, slug, subject: directSubject, html_content: directHtml, to, format: directFormat, wrapper: directWrapper } = req.body;
    ...
    let subject, body, renderOptions = {};
    ...
    if (directSubject && directHtml) {
      ...
      if (template_id) {
        ...
        if (r.rows.length > 0) {
          template = { subject: r.rows[0].subject, body: r.rows[0].html_content, options: {} };
        } else {
    ...
    let finalHtml = await renderStoredEmailHtml(finalBody, renderOptions);
    finalHtml = applyUnsubscribePlaceholder(finalHtml, encodeURIComponent(sampleVars.customer_email));
    await sendEmail(recipient, `[TEST] ${finalSubject}`, finalHtml);
...
<div style="text-align:center;margin:28px 0;"><a href="{portal_link}" style="background:#2e403d;color:#c9dd80;padding:14px 36px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;display:inline-block;">View Your Portal</a></div>`
    },
    {
      ...
      body: `<div style="font-size:12px;font-weight:700;letter-spacing:0.8px;text-transform:uppercase;color:#6b8f47;margin:0 0 12px;">Welcome</div>
<h2 style="color:#2e403d;font-size:28px;font-weight:700;line-height:1.2;margin:0 0 18px;">Your customer portal is ready</h2>
<p style="color:#374151;font-size:15px;line-height:1.8;margin:0 0 16px;">Hi {customer_first_name},</p>
<p style="color:#374151;font-size:15px;line-height:1.8;margin:0 0 16px;">Welcome to Pappas &amp; Co. Landscaping. Your customer portal is ready to use anytime.</p>
<p style="color:#374151;font-size:15px;line-height:1.8;margin:0 0 16px;">To complete your setup, please add a card or bank account on file in your portal.</p>
<p style="color:#374151;font-size:15px;line-height:1.8;margin:0 0 20px;">You can also use the portal to view invoices, estimates, payment history, and account details in one place.</p>
<div style="background:#f8faf6;border-radius:12px;padding:20px 24px;margin:0 0 24px;">
  <p style="margin:0 0 12px;color:#2e403d;font-size:16px;font-weight:700;">Inside Your Portal</p>
  <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="border-collapse:collapse;">
    <tr><td style="padding:12px 0;border-bottom:1px solid #d7dfd1;font-size:14px;font-weight:700;color:#1f2933;vertical-align:top;">Payment Method</td><td style="padding:12px 0 12px 18px;border-bottom:1px solid #d7dfd1;font-size:14px;line-height:1.6;color:#425466;vertical-align:top;">Add a card or bank account on file to complete your setup</td></tr>
    <tr><td style="padding:12px 0;border-bottom:1px solid #d7dfd1;font-size:14px;font-weight:700;color:#1f2933;vertical-align:top;">Invoices</td><td style="padding:12px 0 12px 18px;border-bottom:1px solid #d7dfd1;font-size:14px;line-height:1.6;color:#425466;vertical-align:top;">View balances and payment activity</td></tr>
    <tr><td style="padding:12px 0;border-bottom:1px solid #d7dfd1;font-size:14px;font-weight:700;color:#1f2933;vertical-align:top;">Estimates</td><td style="padding:12px 0 12px 18px;border-bottom:1px solid #d7dfd1;font-size:14px;line-height:1.6;color:#425466;vertical-align:top;">Review estimates and approvals</td></tr>
    <tr><td style="padding:12px 0;font-size:14px;font-weight:700;color:#1f2933;vertical-align:top;">Work Requests</td><td style="padding:12px 0 12px 18px;font-size:14px;line-height:1.6;color:#425466;vertical-align:top;">Submit a new request anytime</td></tr>
  </table>
</div>
<div style="text-align:center;margin:28px 0;"><a href="{portal_link}" style="background:#2e403d;color:#c9dd80;padding:14px 36px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;display:inline-block;">Open Customer Portal</a></div>
<p style="color:#374151;font-size:15px;line-height:1.8;margin:0;">Questions? Call {company_phone} or email {company_email} and we&rsquo;ll be happy to help.</p>`
    },
    {
      id: 'service-reminder',
  ...
  try {
    ...
    for (const cust of customers.rows) {
      try {
        ...
        const baseUrl = process.env.BASE_URL || 'https://app.pappaslandscaping.com';
        let finalHtml = await renderStoredEmailHtml(body, {
          wrapper: tmpl.options?.wrapper || 'full',
          format: tmpl.options?.format,
          showSignature: tmpl.options?.showSignature,
          showFeatures: tmpl.options?.showFeatures,
        });
        finalHtml = applyUnsubscribePlaceholder(finalHtml, vars.unsubscribe_email || '');
        finalHtml = appendHtmlBeforeBodyEnd(finalHtml, `<img src="${baseUrl}/api/t/${trackingId}/open.png" width="1" height="1" style="display:none;" />`);
...
// Core startup schema helpers live in lib/startup-schema.js so the same DB bootstrap path can run from both the app and CLI scripts.
...
app.post('/api/service-complete-email', async (req, res) => {
  try {
    ...
    if (technicianName && /closed by/i.test(technicianName)) {
      ...
      if (match) {
        parsedTechName = ((match[1] || '').trim().split(/\s+/)[0] || technicianName);
        if (!serviceTime) parsedTime = match[2];
      }
    } else if (technicianName) {
      // "Rob Mowing Crew" → "Rob", or "Robert Ellison" → "Robert"
      parsedTechName = ((technicianName || '').trim().split(/\s+/)[0] || technicianName);
    }
