...
function toSummary(invoice) {
  ...
  const metadataThisInvoice = parseNumericCandidate(metadata.this_invoice);
  const storedOutstandingIsPriorBalance = (
    metadataOutstanding !== null
    && metadataOutstanding > 0
    && metadataThisInvoice !== null
    && Math.abs(metadataThisInvoice - financials.total) <= 0.02
    && (
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .window-recipient {
      position: absolute;
      left: ${MAIL_WINDOW_SPEC.recipientZone.leftIn}in;
      top: ${MAIL_WINDOW_SPEC.recipientZone.topIn}in;
