...
function extractExplicitZip(text, { allowBare = false } = {}) {
  const source = String(text || '');
  if (allowBare && /^\s*\d{5}(?:-\d{4})?\s*$/.test(source)) return source.trim().slice(0, 5);
  const zipRe = /\b\d{5}(?:-\d{4})?\b/g;
  let match;
  while ((match = zipRe.exec(source))) {
    const candidate = match[0].slice(0, 5);
    const before = source.slice(Math.max(0, match.index - 32), match.index).toLowerCase();
    const hasZipLabel = /\b(?:zip(?:\s*code)?|postal(?:\s*code)?)\s*(?:(?:is|of)\s*|[:#-]\s*)?$/.test(before);
    // Free-form messages contain house numbers, invoice numbers, and dates. Only a
    // clearly labeled ZIP can trigger an automatic service-area decision.
    if (!hasZipLabel) continue;
    return candidate;
  }
  return null;
}
