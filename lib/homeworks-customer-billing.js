...
async function loadHomeworksCustomerBilling({ customer, getCopilotToken, fetchImpl = fetch }) {
  ...
  const result = await context.query('CustomerBillingRecords',
    'query CustomerBillingRecords($customerId: SafeInt!) { customer(customerId: $customerId) { id outstanding credit } invoices(where: { customerId: { equals: $customerId }, isArchived: false, deletedState: { equals: ACTIVE } }, take: 1000, orderBy: [{ date: desc }]) { id number date status subtotal tax total paidAmount lateFee processingFee isSent isArchived dueDate daysPastDue lineItems { name description } } payments(where: { customerId: { equals: $customerId }, isDeleted: false }, take: 1000, orderBy: [{ date: desc }]) { id invoiceId date invoiceAmount totalAmount creditAmount isRefund method methodDisplayName notes details } }',
  ...
  const payments = result.payments.map(payment => ({
    ...
    method: payment.isRefund ? 'Refund' : payment.methodDisplayName || (payment.method ? payment.method.toLowerCase().replace(/_/g, ' ').replace(/^./, value => value.toUpperCase()) : 'Payment'), notes: payment.notes, details: payment.details,
