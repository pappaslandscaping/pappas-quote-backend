...
function extractMoneyNearLabel(text, label) {
  const match = String(text || '').match(new RegExp(`${label}[\\s\\S]{0,80}?([0-9][0-9,]*\\.\\d{2})`, 'i'));
...
function parseCopilotCustomerStandingHtml(html) {
  ...
  const pastDue = extractMoneyNearLabel(bodyText, 'Past Due');
  const outstanding = extractMoneyNearLabel(bodyText, 'Outstanding');
  const paid = extractMoneyNearLabel(bodyText, 'Paid');
  const credit = extractMoneyNearLabel(bodyText, 'Credit');
