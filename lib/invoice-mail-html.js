...
const { MAIL_WINDOW_SPEC } = require('./mail-window-spec');
  ...
  return `
    <section class="page stub-page">
      <div class="stub-sheet">
        ...
        <div class="copilot-stub">
          <div class="stub-left">
            <div class="stub-address-block">
              ${customerLines.map((line, index) => index === 0
                ? `<strong>${escapeHtml(line)}</strong>`
                : `<div>${escapeHtml(line)}</div>`).join('')}
            </div>

            <div class="stub-address-block remit-block">
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .copilot-stub {
      display: grid;
      grid-template-columns: 2.6in 1.75in 1.3in;
      gap: 0.34in;
      align-items: end;
      min-height: 2.08in;
    }
